...
TEST(TestArchiveReader, Extract) {
    EXPECT_TRUE(ArchiveReader::extract(PROJECT_PATH "/test/Data/archive.zip", PROJECT_BINARY_PATH "/archive-p", "password123"));
}

TEST(TestArchiveReader, Info) {
    auto file = File::create(PROJECT_PATH "/test/Data/archive.zip", BINARY_READ_EXISTED);
