...
void HttpsConnectionImpl::writeBlock(const char *buffer, size_t length, const std::function<void(const asio::error_code &code)> &callback) {
...
void HttpsConnectionImpl::readHeader() {
...
void HttpsConnectionImpl::readBody() {
    node = std::make_unique<sese::iocp::IOBufNode>(MTU_VALUE);
    this->stream->async_read_some(asio::buffer(node->buffer, MTU_VALUE), [conn = getPtr()](const asio::error_code &error, std::size_t bytes_transferred) {
        if (error) {
            // 出现错误，应该断开连接
...
void HttpsConnectionImpl::handleRequest() {
...
void HttpsConnectionImpl::writeHeader() {
...
void HttpsConnectionImpl::writeBody() {
    auto l = std::min<size_t>(this->expect_length - this->real_length, MTU_VALUE);
