...
// GCOVR_EXCL_START
void SHA256Util::encode(const InputStream::Ptr &input, const OutputStream::Ptr &output) noexcept {
    encode(input.get(), output.get());
}
// GCOVR_EXCL_STOP

void sese::SHA256Util::encode(InputStream *input, OutputStream *output) noexcept {
    int64_t size;
    uint64_t total = 0;
    Bitset32 buffer[64];
...
std::unique_ptr<char[]> SHA256Util::encode(const InputStream::Ptr &input, bool is_cap) noexcept {
    return encode(input.get(), is_cap);
}

std::unique_ptr<char[]> sese::SHA256Util::encode(InputStream *input, bool is_cap) noexcept {
    ByteBuilder dest(64);
    encode(input, &dest);
